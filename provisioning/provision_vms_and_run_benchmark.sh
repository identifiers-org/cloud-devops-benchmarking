...
pending_regions=( europe-west4 us-central1 australia-southeast1 asia-east1 asia-northeast1 asia-south1 )
regions=( europe-west4 us-central1 asia-east1 asia-northeast1 asia-south1 )
...
for region in "${regions[@]}"; do
    ...
    created_vms=( "${created_vms[@]}" "${vm_name}" )
...
for vm_name in "${created_vms[@]}"; do
    ...
    scp -o "StrictHostKeyChecking no" -o "UserKnownHostsFile /dev/null" ${ip_address}:~/app/reports/* "${folder_reports}"/.
    vm_zone=`gcloud compute instances list --filter=${vm_name} --format=yaml | grep zone:`
    vm_zone="${vm_zone##*/}"
